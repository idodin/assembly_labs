...
#define __TIMERINT
	...
	int timerstart = 0;
	while (1) {
		// Timer poll flag && timer is started.
		if (HPS_TIM_read_INT_ASM(TIM0) && timerstart) {
			...
			if (micros >= 1000) {
				micros -= 1000;
				seconds++;
				//We reach 60 seconds, reset second count and increment minutes
				if (seconds >= 60) {
					seconds -= 60;
					minutes++;
					// We reach 60 minutes, go back to 0.
					if (minutes >= 60) {
						minutes = 0;
					}
				}
			...
			HEX_write_ASM(HEX0, ((micros % 100) / 10));
			HEX_write_ASM(HEX1, (micros / 100));
